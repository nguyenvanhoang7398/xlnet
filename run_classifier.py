...
class StanceProcessor(GLUEProcessor):
    def __init__(self):
        super(StanceProcessor, self).__init__()
        self.label_column = 3
        self.text_a_column = 1
        self.text_b_column = 2

    def get_labels(self):
        return ["support", "deny", "comment"]
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if i == 0 and self.test_contains_header and set_type == "test":
                continue
            guid = "%s-%s" % (set_type, i)
            if len(line) <= max(self.text_a_column, self.text_b_column, self.label_column):
                tf.logging.warning('Incomplete line, ignored.')
                continue
            text_a, text_b = line[self.text_a_column], line[self.text_b_column]
